  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-background/95 backdrop-blur-md shadow-[0_4px_30px_-10px_rgba(0,0,0,0.4)]"
          : "bg-transparent"
      }`}
    >
        ...
        <div className="hidden lg:flex items-center gap-10">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`font-playfair text-xs tracking-[0.15em] uppercase transition-colors duration-300 relative
                ...
                ${
                  location.pathname === link.path
                    ? "text-primary after:bg-primary after:scale-x-100 after:origin-left"
        ...
        <div className="bg-background/98 backdrop-blur-md border-t border-border px-6 py-8 flex flex-col gap-6">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`font-playfair text-lg tracking-wider uppercase transition-colors duration-300 ${
                location.pathname === link.path
                  ? "text-primary"
                  : "text-foreground/70"
              }`}
          ...
          <div className="flex gap-5 pt-4 border-t border-border">
            ...
            <a href="#" className="text-muted-foreground hover:text-primary transition-colors duration-300" aria-label="WhatsApp">
